...
const IndividualNetScoreboard: React.FC<Props> = ({
  teams, rounds, players, groups, groupPlayers, holeScores, games,
}) => {
  ...
  if (startedRounds.length === 0) {
    return (
      <Card><CardContent className="p-6 text-center text-muted-foreground">Waiting for Round 1 to begin</CardContent></Card>
    );
  }
  ...
  return (
    <Card className="overflow-hidden">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-14 text-center text-xs uppercase tracking-wider font-bold">Net</TableHead>
      ...
      {hasOverrides && <p className="px-4 py-2 text-xs text-muted-foreground border-t">* Contains score adjusted by tournament admin</p>}
    </Card>
  );
};

export default IndividualNetScoreboard;
