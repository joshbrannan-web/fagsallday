...
import { Crown, Trophy, TrendingDown, Minus, Home } from 'lucide-react';
...
// FBO Segment Results Component (copied from Scorecard)
      ...
      <div className="flex-1 overflow-x-auto p-4">
                      ...
                      <td key={h.number} className="p-2 border-r border-border/50">
                        {hasPress ? (
                          <div className="flex flex-col items-center gap-0.5">
                            {presses.oneBall && <span className="text-[10px] font-bold text-amber-600">1B</span>}
                            {presses.twoBall && <span className="text-[10px] font-bold text-amber-600">2B</span>}
                          </div>
                        ) : (
        ...
        {fboGame && fboPlayers.length > 0 && (
          <div className="mt-4 inline-block min-w-full bg-card rounded-xl shadow-sm border border-brand-gold/30 overflow-hidden">
            ...
            <table className="w-full text-center border-collapse text-sm">
              <thead>
                <tr className="bg-muted/50 text-xs font-bold text-muted-foreground">
                  <th className="p-2 text-left min-w-[100px] sticky left-0 bg-muted/50 border-r border-border z-10">Player</th>
                  {activeHoles.map(h => (
                    <th key={h.number} className="p-2 min-w-[40px] border-r border-border/50">{h.number}</th>
                  ))}
                  <th className="p-2 min-w-[50px]">{viewMode === 'FRONT' ? 'F9' : 'B9'}</th>
                  <th className="p-2 min-w-[50px] bg-brand-gold/10">Tot</th>
                </tr>
              </thead>
              <tbody>
                {fboPlayers.map((player, idx) => (
                  <tr key={player.id} className={idx % 2 === 0 ? 'bg-card' : 'bg-muted/30'}>
                    <td className="p-2 text-left font-semibold sticky left-0 bg-inherit border-r border-border z-10">
                      {player.name}
                    </td>
                    {activeHoles.map(h => {
                      const hasDot = getDotsForHole(h.number).includes(player.id);
                      return (
                        <td key={h.number} className="p-2 border-r border-border/50">
                          {hasDot ? (
                            <span className="inline-block w-5 h-5 bg-brand-gold rounded-full text-white text-xs leading-5 font-bold">•</span>
                          ) : (
                            <span className="text-muted-foreground">-</span>
                          )}
                        </td>
                    ...
                    <td className="p-2 font-bold">{getTotalDotsForPlayer(player.id, activeHoles)}</td>
                    <td className="p-2 font-bold bg-brand-gold/10">{getOverallDotsForPlayer(player.id)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-center border-collapse text-sm">
              <thead>
                <tr className="bg-muted/50 text-xs font-bold text-muted-foreground">
                  ...
                  <th className="p-2 min-w-[40px]">S1</th>
                  <th className="p-2 min-w-[40px]">S2</th>
                  <th className="p-2 min-w-[40px]">S3</th>
                  <th className="p-2 min-w-[50px] bg-primary/10">Tot</th>
                </tr>
              </thead>
              <tbody>
                {currentRound.players.map((player, idx) => (
                  <tr key={player.id} className={idx % 2 === 0 ? 'bg-card' : 'bg-muted/30'}>
                    ...
                    {activeHoles.map(h => {
                      ...
                      return (
                        <td key={h.number} className="p-2 border-r border-border/50">
                          {dotCount > 0 ? (
                            <span className="inline-block min-w-5 h-5 px-1 bg-primary rounded-full text-primary-foreground text-xs leading-5 font-bold">
                              {dotCount}
                            </span>
                          ) : (
                            <span className="text-muted-foreground">-</span>
                    ...
                    <td className="p-2 font-bold">{getStockton6StretchDots(player.id, 1)}</td>
                    <td className="p-2 font-bold">{getStockton6StretchDots(player.id, 2)}</td>
                    <td className="p-2 font-bold">{getStockton6StretchDots(player.id, 3)}</td>
                    <td className="p-2 font-bold bg-primary/10">{getStockton6TotalDots(player.id)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {fboGame && fboPlayers.length > 0 && (
          <FBOSegmentResults
            ...
            courseHoles={currentRound.course.holes}
          />
        )}
      </div>
    </div>
